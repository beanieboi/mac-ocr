...
**Node wrapper logic** (arg/env construction, stream parsing, exit classification) is tested against scripted shim binaries — see `Tests/specs/wrapper.ts`. The wrapper resolves its binary relative to its own module URL, so `importWrapper` (`Tests/utils.ts`) copies `src/` into a temp fixture with a shim planted at `bin/mac-ocr` and imports the copy; no Vision is involved.
