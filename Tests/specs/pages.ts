...
await describe('ocr.pages', async () => {
	await test('streams each page in order', async () => {
		const pages = [];
	...
	await test('Array.fromAsync collects every page into an array', async () => {
	...
	await test('rejects re-consuming the same result', async () => {
	...
	await test('is not awaitable directly (plain AsyncIterable)', () => {
		const pages = ocr.pages(fixtureData('multipage.pdf'));
